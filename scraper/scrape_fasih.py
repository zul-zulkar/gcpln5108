...
async def _select_region_with_retry(page, selector, text, attempts=3):
    """Pilih dropdown region (UPI/UP3) dengan retry — dropdown kadang belum render
    saat sidebar baru pertama dibuka (animasi slide-in)."""
    for att in range(attempts):
        try:
            r = await ngx_select(page, selector, text)
            if r:
                return r
        except Exception as e:
            print(f"    [region retry {att+1}/{attempts}] {selector}: {e}")
        await page.wait_for_timeout(700)
        # pastikan sidebar masih terbuka sebelum coba lagi
    ...
    print(f"    [WARN] region select gagal: {selector} -> {text}")
    return None
...
async def scrape_survey(page, survey_name, url, petugas_list, username, password, upi_text, up3_text, stop_event=None):
    ...
    # Beri waktu ngx-select di dalam sidebar selesai render (animasi slide-in)
    await page.wait_for_timeout(800)

    await _select_region_with_retry(page, 'ngx-select[name="region1Id"]', upi_text)
    await _select_region_with_retry(page, 'ngx-select[name="region2Id"]', up3_text)
...
def save_combined_results(all_results):
    """Simpan semua survey dalam 1 file Excel dengan 2 sheet: Pascabayar & Prabayar."""
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_path = os.path.join(OUTPUT_DIR, f"rekap_fasih_{timestamp}.xlsx")
    ...
    sheet_map = [("pascabayar", "Pascabayar"), ("prabayar", "Prabayar")]
    for key, sheet_title in sheet_map:
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        await page.goto(SURVEY_LIST_URL, wait_until="domcontentloaded", timeout=90000)
...
async def main_with_stop(stop_event, input_file=None, username=None, password=None,
                         ...
                         cdp_url=None, surveys_filter=None, petugas_limit=None):
    """Entry point untuk GUI — menerima parameter dinamis dan stop_event.

    cdp_url        : jika diisi (mis. 'http://127.0.0.1:9222'), nempel ke Chrome yang
                     sudah login manual lewat CDP (lolos WAF) alih-alih launch sendiri.
    surveys_filter : list key survey yang dijalankan (mis. ['pascabayar']); None = semua.
    petugas_limit  : batasi jumlah petugas (untuk uji bertahap); None = semua.
    """
    ...
    if petugas_limit:
        petugas_list = petugas_list[:petugas_limit]
    print(f"[INFO] {len(petugas_list)} petugas loaded dari {_input}")
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    surveys = {k: v for k, v in SURVEYS.items()
               if not surveys_filter or k in surveys_filter}

    async with async_playwright() as p:
        if cdp_url:
            # ── Mode CDP: nempel ke Chrome login-manual (anti-WAF) ──────────────
            print(f"[INFO] Connect ke Chrome via CDP: {cdp_url}")
            ...
            print(f"[INFO] Pakai tab login: {page.url}")
        else:
        ...
        if cdp_url:
            # JANGAN tutup browser milik user — cukup putuskan koneksi CDP
            print("[INFO] Selesai (CDP) — browser dibiarkan terbuka.")
        else:
            await browser.close()
